...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
    ...
    for (i=1; i<SIZE-1; i+=1) {
		...
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			conv_horiz += input[(i - 1)*SIZE + j - 1] * horiz_operator[0][0];
			conv_horiz += input[(i - 1)*SIZE + j] * horiz_operator[0][1];
			conv_horiz += input[(i - 1)*SIZE + j + 1] * horiz_operator[0][2];
		
			conv_horiz += input[(i)*SIZE + j - 1] * horiz_operator[1][0];
			conv_horiz += input[(i)*SIZE + j] * horiz_operator[1][1];
			conv_horiz += input[(i)*SIZE + j + 1] * horiz_operator[1][2];
		
			conv_horiz += input[(i + 1)*SIZE + j - 1] * horiz_operator[2][0];
			conv_horiz += input[(i + 1)*SIZE + j] * horiz_operator[2][1];
			conv_horiz += input[(i+1)*SIZE + j + 1] * horiz_operator[2][2];
			//
			conv_vert += input[(i - 1)*SIZE + j - 1] * vert_operator[0][0];
			conv_vert += input[(i - 1)*SIZE + j] * vert_operator[0][1];
			conv_vert += input[(i - 1)*SIZE + j + 1] * vert_operator[0][2];
		
			conv_vert += input[(i)*SIZE + j - 1] * vert_operator[1][0];
			conv_vert += input[(i)*SIZE + j] * vert_operator[1][1];
			conv_vert += input[(i)*SIZE + j + 1] * vert_operator[1][2];
		
			conv_vert += input[(i + 1)*SIZE + j - 1] * vert_operator[2][0];
			conv_vert += input[(i + 1)*SIZE + j] * vert_operator[2][1];
			conv_vert += input[(i+1)*SIZE + j + 1] * vert_operator[2][2];
